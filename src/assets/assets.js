...
const Assets = {
  ...
  all({ hotModule } = {}) {
    ...
    const streams = [
      'config',
      'html',
      !hotModule && 'javascript',
      'sass',
      'images'
    ]
      .map(
        asset => asset && assets[asset] !== false && Assets[asset]({ watch })
      )
      .filter(Boolean)
    ...
    return stream.pipe(
      through2.obj(function(file, enc, callback) {
        ...
        try {
          ...
          const stylesheetPaths = stylesheets.map(f =>
            assetPath(f, assetConfig)
          );
          const scriptPaths = [
            '/config.js',
  ...
  config() {
    ...
    return from(function() {
      const configContents = new File({
        path: 'config.js',
        contents: new Buffer(
          `window.${globalNamespace} = {config: ${configOptionsJSON}}`
        )
      });
  ...
  tasks: {
    cleanAssets(done) {
      ...
      del([`${buildDirectory}/*`, `!${buildDirectory}/.gitkeep`]).then(
        () => done(),
        done
      );
    },
    ...
    cleanAssetsHtml(done) {
      const { buildDirectory } = Assets.installOptions;
      const htmlBuildDirectory =
        Assets.installOptions.htmlBuildDirectory || buildDirectory;
      del([`${htmlBuildDirectory}/index.html`]).then(() => done(), done);
    },

    assets() {
      const stream = Assets.all();
      const { buildDirectory, gulp } = Assets.installOptions;
    ...
    assetsHtml(done) {
      ...
      const {
        buildDirectory,
        htmlBuildDirectory = buildDirectory
      } = Assets.installOptions;
      let once = false;
      Assets.html({ watch })
        .pipe(gulp.dest(htmlBuildDirectory))
        .pipe(
          spy.obj(function(chunk) {
            if (once) return;
